...
import { DEFAULT_AGGREGATION, type MetricSection } from '../config';
import {
  configBySeriesKey,
  findLastUpdatedAt,
  metricPoints,
...
type UseMetricHistorySeriesParams = {
...
export const useMetricHistorySeries = ({
  numericServerId,
  isValidServerId,
  sections,
  range,
  ...
  React.useEffect(() => {
    ...
    const loadMetrics = async () => {
      const successfulRequests: Array<{ seriesKey: string; requestKey: string }> = [];
      let aborted = false;
      ...
      try {
        ...
        if (
          results.some(
            (result) => result.status === 'rejected' && isCanceledRequestError(result.reason),
          )
        ) {
          aborted = true;
          return;
        ...
        results.forEach((result, index) => {
          ...
          successfulRequests.push({ seriesKey, requestKey });
        ...
        if (failedSeriesKeys.length > 0) {
          const hasLoadedHistory =
            loadedSeriesKeys.size > 0 || hasLoadedRequests(loadedRequestKeysRef.current);
        ...
        if (successfulRequests.length > 0) {
          successfulRequests.forEach(({ seriesKey, requestKey }) => {
