...
func TestIntegrationUpdateSessionRejectsLoginForReplacedChannel(t *testing.T) {
	...
	if err := st.UpdateMTProtoSession(ctx, stored.ID, originalRevision, "stale-session"); !errors.Is(err, alertstore.ErrChannelVersionStale) {
		t.Fatalf("UpdateMTProtoSession() error = %v, want ErrChannelVersionStale", err)
	}
