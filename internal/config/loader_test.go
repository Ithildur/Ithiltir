...
func TestCompilePublicURL(t *testing.T) {
	tests := []struct {
		...
		{name: "punycode domain", raw: "https://xn--fsqu00a.xn--0zwm56d", want: "https"},
		...
		{name: "shell substitution", raw: "https://dash$(id).example"},
		{name: "shell separator", raw: "https://dash;reboot.example"},
		{name: "unicode domain", raw: "https://例子.测试"},
		{name: "invalid dns label", raw: "https://-dash.example.com"},
		{name: "zero port", raw: "https://dash.example.com:0"},
		{name: "oversized port", raw: "https://dash.example.com:65536"},
