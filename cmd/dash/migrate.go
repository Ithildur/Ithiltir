...
func runMigrate(args []string) {
	...
	if err != nil {
		infra.Log().Error(ctx, "load config failed", err)
		os.Exit(1)
	}
	...
	if _, err := infra.InitLogger(logLevel, cfg.App.LogFormat); err != nil {
		infra.Log().Error(ctx, "init logger failed", err)
		os.Exit(1)
	}
	db, err := infra.NewGORMTimescale(ctx, cfg.Database)
	if err != nil {
		infra.Log().Error(ctx, "init timescale failed", err)
		os.Exit(1)
	}
	sqlDB, err := db.DB()
	if err != nil {
		infra.Log().Error(ctx, "extract sql.DB failed", err)
		os.Exit(1)
	}
	defer sqlDB.Close()
	...
	if err != nil {
		infra.Log().Error(ctx, "resolve notification config key path failed", err)
		os.Exit(1)
	}
	res, err := kitmigration.Run(ctx, migrate.New(sqlDB, notifyKeyPath))
	if err != nil {
		infra.Log().Error(ctx, "migrate failed", err)
		os.Exit(1)
	}
