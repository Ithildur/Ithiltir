...
- `PATCH /api/admin/nodes/{id}` trims `secret` and returns `400 invalid_secret` unless it contains 1–128 Unicode characters. It returns `409 duplicate_secret` when the submitted `secret` already belongs to another node.
...
- `GET /api/admin/alerts/channels` and `GET /api/admin/alerts/channels/{id}` return sanitized channel configuration plus delivery health. `delivery_status` is `unknown` before the first successful delivery, `healthy` after a success with no current blocked work, `degraded` while delivery failures or blocked notifications remain, and `disabled` while the channel is disabled.
