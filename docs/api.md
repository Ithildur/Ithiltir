...
- `PATCH /api/admin/nodes/{id}` accepts `traffic_p95_enabled`, `tags`, and node traffic override fields. Omitted non-cycle fields are unchanged. Node cycle override fields are atomic: if any of `traffic_cycle_mode`, `traffic_billing_start_day`, `traffic_billing_anchor_date`, or `traffic_billing_timezone` is submitted, the request must include `traffic_cycle_mode` and every field used by that mode, otherwise it returns `400 invalid_traffic_cycle_settings`. `default` uses no billing fields; `calendar_month` uses `traffic_billing_timezone`; `clamp_to_month_end` uses `traffic_billing_start_day` and `traffic_billing_timezone`; `whmcs_compatible` uses `traffic_billing_anchor_date` and `traffic_billing_timezone`, with `traffic_billing_start_day` derived from the anchor date. `tags` accepts a string array; values are trimmed, empty values and duplicates are removed, and `[]` clears tags. `traffic_cycle_mode` allows `default`, `calendar_month`, `whmcs_compatible`, and `clamp_to_month_end`; `traffic_direction_mode` allows `default`, `out`, `both`, and `max`.
...
- `POST /api/admin/nodes/{id}/traffic/rebuild` starts an in-process rebuild of that node's 5-minute traffic facts within the configured `database.traffic_retention_days` window from stored NIC metrics. It returns `202` with the same status body. Missing or deleted nodes return `404 not_found`; any running rebuild returns `409 traffic_rebuild_running`. The task rewrites only 5-minute facts and invalidates overlapping monthly snapshots for that retained window when it starts. If the task fails, affected monthly history is regenerated later by normal snapshot maintenance, computed from retained facts when no reusable snapshot exists, or restored by retrying the rebuild; data outside retention is not recovered automatically by the database.
...
- `dash_update_mode=notify` makes Dash periodically check the configured update channel and send an update-available message through enabled notification channels. `dash_update_mode=auto` checks periodically, starts an update when a newer version is available, and sends start/result notifications.
...
- Traffic reads use the node's effective traffic configuration. Nodes with `traffic_cycle_mode=default` inherit the global cycle mode, billing start day, anchor date, and timezone; other nodes use their own `traffic_*` cycle fields. Nodes with `traffic_direction_mode=default` inherit the global direction mode; other nodes use their own direction override.
