...
- 从 `0.3.2` 升级会新增迁移 `0014` 并开始采集 Uptime 历史，不会自动追溯升级前的在线率。已有节点及指标历史保持不变。
- 早期 `0.3.3` 预发布版使用 `node_online.online` 和 `samples`；正式版使用 `online_ms` 和 `observed_ms`，小时聚合同步改为时长之和。旧布尔值不能还原分钟内的在线时长，不提供旧格式适配或伪造转换。
- 仍使用布尔版 `0014` 的测试实例需要停止 Dash，备份并重建 Uptime 分钟表、小时聚合及其任务，再应用正式版 `0014` 并运行 `dash migrate`。这会重新开始 Uptime 历史，节点及 CPU、内存等指标历史不重建。已使用时长结构的实例保留 Uptime 历史。数据库与 Dash 必须配套更新，Goose 不会自动重跑已经执行的 `0014`。
...
- `dash migrate` 会使用 AES-256-GCM 加密完整通知渠道配置，把当前逻辑行中的明文替换为 `{}`，并且只在数据库尚无密文时创建 `$DASH_HOME/configs/notify-config.key`。该密钥必须与 PostgreSQL 分开备份。密钥缺失、格式错误或无法解密任一存量渠道时，启动会直接失败，不会生成替代密钥或回退到明文。该迁移不提供物理安全擦除：MVCC 死元组、表空闲空间、保留的 WAL、副本、物理备份和存储快照仍可能包含旧明文，必须按对应的存储及备份保留策略显式退役后才能解除敏感数据保护。
- 通知渠道在列表、更新、测试和发送通知时，均按当前严格 schema 校验数据库中的配置。列表和详情读取遇到非法渠道时，会保留该渠道并返回 `config: null`，不会导致整个响应失败。
- `api_id`、`smtp_port` 等整数字段不再接受数字字符串或值为整数的 JSON 浮点数。
- 未知配置字段会被拒绝。
- 不符合当前 schema 的存量配置必须删除后重新创建。管理界面会保留这些渠道以提供该恢复路径，同时禁止不安全的编辑、启用、选择和测试操作。
