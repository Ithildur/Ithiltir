...
- `node_online` 按 `(server_id, minute, online)` 保存节点分钟级状态，每个节点每分钟至多一条非空布尔值记录，时间对齐到整分钟。TimescaleDB 的 `sample_node_online` 任务每分钟根据 `server_current_metrics.collected_at`（最后一次成功上报的服务端接收时间）采样。没有当前指标行的节点和已软删除节点不生成记录；删除节点后，已有历史保留至过期清理。最后接收时间处于 `app.node_offline_threshold`（默认 17 秒）以内即为在线。迁移和启动时将该阈值同步到数据库任务，Go 不运行采样循环。同一分钟以第一次采样为准，重试不覆盖已有结果。布尔值表示采样时刻的状态，不代表整分钟持续可达。
- `node_online` 缺少记录表示未知，任务不会补写错过的分钟或更早历史。只要 PostgreSQL 可用，即使 Dash 停止，采样仍继续，上报过期后记录离线；因此它衡量的是上报可用性，无法区分节点、Dash 或网络链路故障。时序表按一天分块，保留 46 天，由策略任务按整个分块清理。任务调度与失败可通过 `timescaledb_information.jobs`、`job_stats`、`job_errors` 查看。现有在线率 API 仍读取 `server_online_30m`。预留的 `services` 和 `service_checks` 表与该存储独立。
- `node_online_1h` 是启用实时合并的小时连续聚合，保存 `samples`（已记录分钟数）和 `online`（在线分钟数）。归集策略在当地每个整点后 5 秒执行，物化已完成小时，最新尾部由 TimescaleDB 实时合并。分钟原表和小时聚合均使用 46 天保留策略。`/api/metrics/uptime` 将小时计数汇总为 45 个日历日，`/api/metrics/uptime/day` 读取同一小时视图。缺失观测不计入分母；普通完整小时有 60 个样本，不完整小时、采样中断、夏令时切换时可能不同。迁移和启动将小时桶对齐到 `app.timezone`；修改时区仅重建派生的小时视图。未命名的本地时区需能从宿主机时区配置解析，否则须显式配置 IANA `app.timezone`。
