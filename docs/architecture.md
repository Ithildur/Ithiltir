...
- Dash self-update state is local runtime state under `DASH_HOME/runtime/dash-update`. The admin update endpoint starts the packaged Linux updater through a systemd transient unit; status and log tail come from local files, not PostgreSQL or Redis. Version checks read Dash release tags from the configured Git remote.
