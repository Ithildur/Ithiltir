...
- TCP/UDP connection counts are persisted numeric metrics. They are written to `tcp_conn` and `udp_conn` and remain available to history queries as `conn.tcp` and `conn.udp`. On Linux, full host/netns connection counts come from the root-side connections cache because the agent runs with low privileges. If the cache is missing or stale, such as after a binary-only Dash upgrade, the agent keeps reporting through the direct low-privilege collection path, but that path may see only the connections available to the agent.
